...
type ClientEnd struct {
	...
	ch      chan reqMsg // copy of Network.endCh 所有clientend共享同一个Network的channel， Network框架监听这个channel来处理call请求
}
...
func (e *ClientEnd) Call(svcMeth string, args interface{}, reply interface{}) bool {
	...
	//将请求包放入Network框架的监听请求包的channel中去，
	//Network框架收到请求包后，会生成一个goroutine来调用ProcessReq处理请求
	e.ch <- req

	//NetWork处理完请求，会把回复放入请求的replyCh channel中
	rep := <-req.replyCh
	if rep.ok {
...
type Network struct {
	...
	//节点名字endname和ClientEnd的映射对
	ends map[interface{}]*ClientEnd // ends, by name

	//该节点对应的网络是否打开， 根据节点名字endname索引
	enabled map[interface{}]bool // by end name

	//根据raft的索引号获取相应的server结构体
	servers map[interface{}]*Server // servers, by name

	//根据连接名字查raft的名字（即raft的索引）
	connections map[interface{}]interface{} // endname -> raft的索引号
	endCh       chan reqMsg
}
...
//reliable ： 用来模拟网络是否可靠，如果值为false，那么就模拟90%的丢包率
func (rn *Network) ProcessReq(req reqMsg) {
	enabled, servername, server, reliable, longreordering := rn.ReadEndnameInfo(req.endname)

	//如果raft peer连接存在并且正常连接
	if enabled && servername != nil && server != nil {
		...
		if replyOK == false || serverDead == true {
			...
			log.Println("ProcessReq, drop the reply, return as if timeout")
			req.replyCh <- replyMsg{false, nil}
		} else if longreordering == true && rand.Intn(900) < 600 {
			// delay the response for a while
			ms := 200 + rand.Intn(1+rand.Intn(2000))
			time.Sleep(time.Duration(ms) * time.Millisecond)
			log.Println("ProcessReq, delay the response for a while")
			req.replyCh <- reply
		} else {
			req.replyCh <- reply
		}
...
//将一个server添加到网络框架中，servername是raft的索引i（raft名字）
...
func (rs *Server) dispatch(req reqMsg) replyMsg {
	...
	dot := strings.LastIndex(req.svcMeth, ".") //"Raft.AppendEntries"
	serviceName := req.svcMeth[:dot]
	methodName := req.svcMeth[dot+1:]

	//指定raft的service存在指定的serviceName
...
//rcvr： 一个raft结构体
//创建一个service，将输入的rcvr的方法用rpc的方式提供服务。这是一个通用的函数，用来将服务的实现，与网络rpc解耦。
